...
if ( $op == 'editprofile' ) {
	...
	include_once $GLOBALS['xoops']->path( 'include/comment/comment_constants.php' );
    ...
    echo '<a href="userinfo.php?uid=' . $xoopsUser->getVar( 'uid' ) . '">' . _US_PROFILE . '</a>&nbsp;<span class="bold">&raquo;&raquo;</span>&nbsp;' . _US_EDITPROFILE . '<br /><br />';
		...
		if ( $uploader->fetchMedia( $_POST['xoops_upload_file'][0] ) ) {
			...
			if ( $uploader->upload() ) {
				...
				if ( !$avt_handler->insert( $avatar ) ) {
					...
					redirect_header( 'userinfo.php?t=' . time() . '&amp;uid=' . $xoopsUser->getVar( 'uid' ), 1, _US_PROFUPDATED );
				}
			}
		}
