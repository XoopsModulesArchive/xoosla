...
function profile_getFieldForm( &$field, $action = false )
{
    ...
    $fieldcat_id = ( !$field->isNew() ) ? $field->getVar( 'cat_id' ) : 0;
...
function profile_getRegisterForm( &$user, $profile, $step = null )
{
	...
	$reg_form->addElement( new XoopsFormButtonTray( 'submit', _SAVE, 'submit' ) );
    return $reg_form;
}
...
function profile_getUserForm( &$user, $profile = null, $action = false )
{
	...
	$form = new XoopsThemeForm( _US_EDITPROFILE, 'userinfo', $action, 'post', true );
