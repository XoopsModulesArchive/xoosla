...
if ( !is_object( $GLOBALS['xoopsUser'] ) ) {
	redirect_header( XOOPS_URL, 1, _US_NOEDITRIGHT );
	exit();
}
...
if ( $op == 'save' ) {
	if ( !$GLOBALS['xoopsSecurity']->check() ) {
		redirect_header( XOOPS_URL . '/modules/' . $GLOBALS['xoopsModule']->getVar( 'dirname', 'n' ) . '/', 1, _US_NOEDITRIGHT . '<br />' . implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
		exit();
	}
	...
	if ( !empty( $stop ) ) {
		...
		if ( !$member_handler->insertUser( $edituser ) ) {
			...
			redirect_header( XOOPS_URL . '/modules/' . $GLOBALS['xoopsModule']->getVar( 'dirname', 'n' ) . '/userinfo.php?uid=' . $edituser->getVar( 'uid' ), 1, _US_PROFUPDATED );
		}
	}
}
...
if ( $op == 'avatarform' ) {
	...
	$form2->addElement( new XoopsFormButtonTray( 'submit2', _SAVE, 'submit' ) );
...
if ( $op == 'avatarupload' ) {
	if ( !$GLOBALS['xoopsSecurity']->check() ) {
		redirect_header( 'index.php', 1, _US_NOEDITRIGHT . "<br />" . implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
		exit;
	}
	...
	if ( empty( $uid ) || $GLOBALS['xoopsUser']->getVar( 'uid' ) != $uid ) {
		redirect_header( 'index.php', 1, _US_NOEDITRIGHT );
		exit();
	}
		...
		if ( $uploader->fetchMedia( $_POST['xoops_upload_file'][0] ) ) {
			...
			if ( $uploader->upload() ) {
				...
				if ( !$avt_handler->insert( $avatar ) ) {
					...
					redirect_header( 'userinfo.php?t=' . time() . '&amp;uid=' . $GLOBALS['xoopsUser']->getVar( 'uid' ), 1, _US_PROFUPDATED );
				}
			}
		}
		redirect_header( 'edituser.php?op=avatarform', 1, $uploader->getErrors() );
	}
}
