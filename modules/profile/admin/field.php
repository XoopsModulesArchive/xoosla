...
switch ( $op ) {
	...
	case 'edit':
		...
		if ( !$obj->getVar( 'field_config' ) && !$obj->getVar( 'field_show' ) && !$obj->getVar( 'field_edit' ) ) { // If no configs exist
			redirect_header( 'field.php', 1, _PROFILE_AM_FIELDNOTCONFIGURABLE );
		}
		include_once( '../include/forms.php' );
		$form = profile_getFieldForm( $obj );
		$form->display();
	...
	case 'reorder':
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'field.php', 1, implode( ',', $GLOBALS['xoopsSecurity']->getErrors() ) );
		}
		if ( isset( $_POST['field_ids'] ) && count( $_POST['field_ids'] ) > 0 ) {
			...
			if ( count( $ids ) > 0 ) {
				...
				if ( count( $errors ) == 0 ) {
					// no errors
					redirect_header( 'field.php', 1, sprintf( _PROFILE_AM_SAVEDSUCCESS, _PROFILE_AM_FIELDS ) );
				} else {
					redirect_header( 'field.php', 1, implode( '<br />', $errors ) );
				}
			}
		}
		break;
	...
	case 'save':
		...
		if ( isset( $_REQUEST['id'] ) ) {
			$obj = $profilefield_handler->get( $_REQUEST['id'] );
			if ( !$obj->getVar( 'field_config' ) && !$obj->getVar( 'field_show' ) && !$obj->getVar( 'field_edit' ) ) { // If no configs exist
				redirect_header( 'admin.php', 1, _PROFILE_AM_FIELDNOTCONFIGURABLE );
			}
		} else {
			$obj = $profilefield_handler->create();
		...
		if ( $profilefield_handler->insert( $obj ) ) {
			...
			redirect_header( $url, 1, sprintf( _PROFILE_AM_SAVEDSUCCESS, _PROFILE_AM_FIELD ) );
		}
	...
	case 'delete':
		$obj = $profilefield_handler->get( $_REQUEST['id'] );
		if ( !$obj->getVar( 'field_config' ) ) {
			redirect_header( 'index.php', 1, _PROFILE_AM_FIELDNOTCONFIGURABLE );
		}
		if ( isset( $_REQUEST['ok'] ) && $_REQUEST['ok'] == 1 ) {
			...
			if ( $profilefield_handler->delete( $obj ) ) {
				redirect_header( 'field.php', 1, sprintf( _PROFILE_AM_DELETEDSUCCESS, _PROFILE_AM_FIELD ) );
			} else {
				echo $obj->getHtmlErrors();
			}
		} else {
