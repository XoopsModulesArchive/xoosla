...
switch ( $op ) {
	...
	case 'save':
		...
		if ( $handler->insert( $obj ) ) {
			redirect_header( 'step.php', 1, sprintf( _PROFILE_AM_SAVEDSUCCESS, _PROFILE_AM_STEP ) );
		}
	...
	case 'delete':
		...
		if ( isset( $_REQUEST['ok'] ) && $_REQUEST['ok'] == 1 ) {
			if ( $handler->delete( $obj ) ) {
				redirect_header( 'step.php', 1, sprintf( _PROFILE_AM_DELETEDSUCCESS, _PROFILE_AM_STEP ) );
			} else {
				echo $obj->getHtmlErrors();
			}
		} else {
