...
switch ( $op ) {
	...
	case 'edit':
		...
		if ( in_array( XOOPS_GROUP_ADMIN, $obj->getGroups() ) && !in_array( XOOPS_GROUP_ADMIN, $GLOBALS['xoopsUser']->getGroups() ) ) {
			...
			redirect_header( 'user.php', 1, _US_NOEDITRIGHT );
		}
		include_once '../include/forms.php';
		$form = profile_getUserForm( $obj );
		$form->display();
	...
	case 'save':
		...
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'user.php', 1, _US_NOEDITRIGHT . '<br />' . implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
			exit;
		}
		// Dynamic fields
		...
		if ( count( $errors ) == 0 ) {
			if ( $handler->insertUser( $user ) ) {
				...
				if ( $user->isNew() ) {
					redirect_header( 'user.php', 1, _PROFILE_AM_USERCREATED, false );
				} else {
					redirect_header( 'user.php', 1, _US_PROFUPDATED, false );
				}
			}
		} else {
	...
	case 'delete':
		if ( $_REQUEST['id'] == $GLOBALS['xoopsUser']->getVar( 'uid' ) ) {
			redirect_header( 'user.php', 1, _PROFILE_AM_CANNOTDELETESELF );
		}
		$obj = $handler->getUser( $_REQUEST['id'] );
		$groups = $obj->getGroups();
		if ( in_array( XOOPS_GROUP_ADMIN, $groups ) ) {
			redirect_header( 'user.php', 1, _PROFILE_AM_CANNOTDELETEADMIN, false );
		}

		if ( isset( $_REQUEST['ok'] ) && $_REQUEST['ok'] == 1 ) {
			if ( !$GLOBALS['xoopsSecurity']->check() ) {
				redirect_header( 'user.php', 1, implode( ',', $GLOBALS['xoopsSecurity']->getErrors() ), false );
			}
			$profile_handler = xoops_getmodulehandler( 'profile' );
			...
			if ( !$profile || $profile->isNew() || $profile_handler->delete( $profile ) ) {
				if ( $handler->deleteUser( $obj ) ) {
					redirect_header( 'user.php', 1, sprintf( _PROFILE_AM_DELETEDSUCCESS, $obj->getVar( 'uname' ) . ' (' . $obj->getVar( 'email' ) . ')' ), false );
				} else {
