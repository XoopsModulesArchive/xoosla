...
if ( $email == '' ) {
	redirect_header( 'user.php', 1, _US_SORRYNOTFOUND, false );
	exit();
}
...
if ( empty( $user ) ) {
	...
	redirect_header( 'user.php', 1, $msg, false );
	exit();
} else {
	...
	if ( $code != '' && $areyou == $code ) {
		...
		redirect_header( 'user.php', 1, sprintf( _US_PWDMAILED, $user->getVar( 'uname' ) ), false );
		exit();
		// If no Code, send it
	} else {
		$xoopsMailer = xoops_getMailer();
