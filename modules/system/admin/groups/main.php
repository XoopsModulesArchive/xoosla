...
switch ( $op ) {
	...
	case 'groups_save_add':
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=groups', 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
		}
	...
	case 'groups_delete':
		...
		if ( $groups_id > 0 ) {
			...
			if ( isset( $_POST['ok'] ) && $_POST['ok'] == 1 ) {
				if ( !$GLOBALS['xoopsSecurity']->check() ) {
					redirect_header( 'admin.php?fct=groups', 1, implode( ',', $GLOBALS['xoopsSecurity']->getErrors() ) );
				}
				if ( $groups_id > 0 && !in_array( $groups_id, array( XOOPS_GROUP_ADMIN, XOOPS_GROUP_USERS, XOOPS_GROUP_ANONYMOUS ) ) ) {
					...
					redirect_header( 'admin.php?fct=groups', 1, _AM_SYSTEM_GROUPS_ERROR_DELETE );
				}
			} else {
				// Define Stylesheet
				$xoTheme->addStylesheet( XOOPS_URL . '/modules/system/css/admin.css' );
