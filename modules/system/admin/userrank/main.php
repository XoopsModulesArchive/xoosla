...
if ( !xoops_getModuleOption( 'active_userrank', 'system' ) ) redirect_header( 'admin.php', 1, _AM_SYSTEM_NOTACTIVE );
// Parameters
...
switch ( $op ) {
	...
	case 'userrank_save':
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=userrank', 1, implode( ',', $GLOBALS['xoopsSecurity']->getErrors() ) );
		}
		...
		if ( $userrank_Handler->insert( $obj ) ) {
			redirect_header( 'admin.php?fct=userrank', 1, _AM_SYSTEM_USERRANK_SAVE );
		}
		break;
		...
		if ( isset( $_POST['ok'] ) && $_POST['ok'] == 1 ) {
			...
			xoops_confirm( 
				array( 
					...
					$_SERVER['REQUEST_URI'], 
					sprintf( 
						_AM_SYSTEM_USERRANK_SUREDEL ) . '<br \>
						<img src="' . XOOPS_UPLOAD_URL . '/' . $rank_img . '" alt="" /><br \>' 
					);
		}
		break;
