 ...
 * Check is active
 */
if ( !xoops_getModuleOption( 'active_comments', 'system' ) ) redirect_header( 'admin.php', 1, _AM_SYSTEM_NOTACTIVE );
// Get Action type
	...
	case 'comments_purge':
		...
		if ( $comments_groupe != '' ) {
			foreach ( $_POST['comments_groupe'] as $del => $u_name ) {
				...
				if ( $mcount > 4000 ) {
					redirect_header( 'admin.php?fct=comments', 1, _MP_DELETECOUNT );
				}
				for ( $i = 0; $i < $mcount; $i++ ) {
		...
		if ( $verif == true ) {
			if ( $comment_handler->deleteAll( $criteria ) ) {
				redirect_header( 'admin.php?fct=comments', 1, _AM_SYSTEM_DBUPDATED );
			}
		} else {
			redirect_header( 'admin.php?fct=comments', 1, _AM_SYSTEM_DBUPDATED );
		}
