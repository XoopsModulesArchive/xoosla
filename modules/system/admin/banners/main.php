...
switch ( $op ) {
	...
	case 'banner_save': // Save banner
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=banners', 1, implode( ',', $GLOBALS['xoopsSecurity']->getErrors() ) );
		}
		...
		if ( $banner_Handler->insert( $obj ) ) {
			redirect_header( 'admin.php?fct=banners', 1, _AM_SYSTEM_BANNERS_DBUPDATED );
		}
		...
		if ( $bid > 0 ) {
			...
			if ( isset( $_POST['ok'] ) && $_POST['ok'] == 1 ) {
				...
				if ( $banner_Handler->delete( $obj ) ) {
					redirect_header( 'admin.php?fct=banners', 1, _AM_SYSTEM_BANNERS_DELEBNR );
				} else {
					xoops_error( $obj->getHtmlErrors() );
				}
			} else {
