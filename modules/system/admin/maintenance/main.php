...
if ( !xoops_getModuleOption( 'active_maintenance', 'system' ) ) {
	redirect_header( 'admin.php', 1, _AM_SYSTEM_NOTACTIVE );
}
// Get Action type
...
switch ( $op ) {
	case 'list': default:
		...
		$form_maintenance->addElement( new XoopsFormButton( '', 'maintenance_save', _SAVE, 'submit' ) );
		// Form Dump
		...
		$form_dump->addElement( new XoopsFormButton( '', 'dump_save', _SAVE, 'submit' ) );
		// Assign form
	...
	case 'maintenance_save':
		...
		if ( $result_cache == false && $result_session == false && $result_maintenance == false ) {
			redirect_header( 'admin.php?fct=maintenance', 1, _AM_SYSTEM_MAINTENANCE_ERROR_MAINTENANCE );
		}
	...
	case 'dump_save':
		...
		if ( ( $dump_tables == true && $dump_modules == true ) || ( $dump_tables == false && $dump_modules == false ) ) {
			redirect_header( 'admin.php?fct=maintenance', 1, _AM_SYSTEM_MAINTENANCE_DUMP_ERROR_TABLES_OR_MODULES );
		}
