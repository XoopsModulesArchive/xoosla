 ...
 * @version main.php 00 27/02/2012 05:14 Catzwolf $Id:
 */
...
switch ( $op ) {
	...
	case 'users_delete':
		...
		if ( isset( $_REQUEST['ok'] ) && $_REQUEST['ok'] == 1 ) {
			if ( !$GLOBALS['xoopsSecurity']->check() ) {
				redirect_header( 'admin.php?fct=users', 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
			}
	...
	case 'action_group':
		if ( ( @isset( $_REQUEST['memberslist_id'] ) || @$_REQUEST['memberslist_id'] != '' ) ) {
			...
			if ( $error != '' ) {
				redirect_header( 'admin.php?fct=users', 1, sprintf( _AM_SYSTEM_USERS_ERROR, $error ) );
			} else {
				redirect_header( 'admin.php?fct=users', 1, _AM_SYSTEM_DBUPDATED );
			}
		}
