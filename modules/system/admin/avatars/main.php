...
// Check is active
if ( !xoops_getModuleOption( 'active_avatars', 'system' ) ) redirect_header( 'admin.php', 1, _AM_SYSTEM_NOTACTIVE );
// Get Action type
...
switch ( $op ) {
	...
	case 'save':
		...
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=avatars', 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
			exit();
		}
		$config_handler = xoops_gethandler( 'config' );
		$xoopsConfigUser = $config_handler->getConfigsByCat( XOOPS_CONF_USER );
		...
		redirect_header( 'admin.php?fct=avatars', 1, _AM_SYSTEM_DBUPDATED );
		break;
	...
	case 'delfileok':
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=avatars', 1, 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
		}
		$avatar_id = system_CleanVars ( $_POST, 'avatar_id', 0, 'int' );
		if ( $avatar_id <= 0 ) {
			redirect_header( 'admin.php?fct=avatars', 1, _AM_SYSTEM_DBERROR );
		...
		if ( isset( $user_id ) && $avatar->getVar( 'avatar_type' ) == 'C' ) {
			$xoopsDB->query( 'UPDATE ' . $xoopsDB->prefix( 'users' ) . ' SET user_avatar='.$this->db->quoteString( 'blank.gif' ).' WHERE uid=' . intval( $user_id ) );
		} else {
			$xoopsDB->query( 'UPDATE ' . $xoopsDB->prefix( 'users' ) . ' SET user_avatar='.$this->db->quoteString( 'blank.gif' ).' WHERE user_avatar='. $this->db->quoteString( $file  ) );
		}
		redirect_header( 'admin.php?fct=avatars', 1, _AM_SYSTEM_DBUPDATED );
		break;
}
