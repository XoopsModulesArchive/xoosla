 ...
 * @package modulesadmin.php
 * @since 1.0.0.0
 * @author John Neill <zaquria@xoosla.com>
 * @version modulesadmin.php 26 2012-02-17 09:16:15Z catzwolf $Id:
 */
			...
			if ( !$module_handler->insert( $module ) ) {
				...
				if ( $blocks != false ) {
					...
					foreach ( $blocks as $blockkey => $block ) {
						...
						$description = addslashes( trim( $block['description'] ) );

						$sql = "INSERT INTO " . $db->prefix( "newblocks" ) . " (bid, mid, func_num, options, name, title, description, content, side, weight, visible, block_type, c_type, isactive, dirname, func_file, show_func, edit_func, template, bcachetime, last_modified) VALUES ($newbid, $newmid, " . intval( $blockkey ) . ", '$options', '" . $block_name . "','" . $block_name . "', '" . $block_description . "', '', 0, 0, 0, 'M', 'H', 1, '" . addslashes( $dirname ) . "', '" . addslashes( trim( $block['file'] ) ) . "', '" . addslashes( trim( $block['show_func'] ) ) . "', '" . addslashes( $edit_func ) . "', '" . $template . "', 0, " . time() . ")";
						if ( !$db->query( $sql ) ) {
