...
switch ( $op ) {
	...
	case 'save':
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=preferences', 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
		}
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				if ( is_array( $new_value ) || $new_value != $config->getVar( 'conf_value' ) ) {
					...
					if ( !$theme_updated && $config->getVar( 'conf_catid' ) == XOOPS_CONF && $config->getVar( 'conf_name' ) == 'theme_set' ) {
						...
						$member_handler->updateUsersByField( 'theme', $ {
								$config->getVar( 'conf_name' )}
							);
						$theme_updated = true;
					}
		...
		if ( isset( $redirect ) && $redirect != '' ) {
			redirect_header( $redirect, 1, _AM_SYSTEM_DBUPDATED );
		} else {
			redirect_header( 'admin.php?fct=preferences', 1, _AM_SYSTEM_DBUPDATED );
		}
		break;
