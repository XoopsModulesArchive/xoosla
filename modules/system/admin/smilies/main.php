...
// Check is active
if ( !xoops_getModuleOption( 'active_smilies', 'system' ) ) redirect_header( 'admin.php', 1, _AM_SYSTEM_NOTACTIVE );
// Parameters
...
switch ( $op ) {
	...
	case 'save_smilie':
		if ( !$GLOBALS['xoopsSecurity']->check() ) {
			redirect_header( 'admin.php?fct=smilies', 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
			exit();
		}
		// Define Breadcrumb and tips
		$xoBreadCrumb->addLink( _AM_SYSTEM_SMILIES_NAV_ADD );
		...
		if ( $smilies_Handler->insert( $obj ) ) {
			redirect_header( 'admin.php?fct=smilies', 1, _AM_SYSTEM_DBUPDATED );
		}
	...
	case 'smilies_delete':
		...
		if ( isset( $_POST['ok'] ) && $_POST['ok'] == 1 ) {
			if ( !$GLOBALS['xoopsSecurity']->check() ) {
				redirect_header( 'admin.php?fct=smilies', 1, implode( ',', $GLOBALS['xoopsSecurity']->getErrors() ) );
			}
