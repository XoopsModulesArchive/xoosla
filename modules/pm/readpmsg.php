...
$xoBreadcrumbs = array();
$xoBreadcrumbs[] = array( 'title' => 'Home', 'link' => XOOPS_URL . '/modules/' . $GLOBALS['xoopsModule']->getVar( 'dirname', 'n' ) . '/' );
...
if ( is_object( $pm ) && ( $pm->getVar( 'from_userid' ) != $GLOBALS['xoopsUser']->getVar( 'uid' ) ) && ( $pm->getVar( 'to_userid' ) != $GLOBALS['xoopsUser']->getVar( 'uid' ) ) ) {
	redirect_header( XOOPS_URL . '/modules/' . $GLOBALS['xoopsModule']->getVar( 'dirname', 'n' ) . '/index.php', 1, _NOPERM );
	exit();
}
...
if ( is_object( $pm ) && !empty( $_POST['action'] ) ) {
	...
	} else if ( !empty( $_REQUEST['move_message'] ) && $_REQUEST['op'] != 'save' && !$GLOBALS['xoopsUser']->isAdmin() && $pm_handler->getSavecount() >= $GLOBALS['xoopsModuleConfig']['max_save'] ) {
	...
	redirect_header( 'viewpmsg.php?op=' . htmlspecialchars( $_REQUEST['op'] ) , 1, $res_message );
}
...
if ( is_object( $pm ) && !empty( $pm ) ) {
	//if ( $pm->getVar( 'from_userid' ) != $GLOBALS['xoopsUser']->getVar( 'uid' ) ) {
		...
		$reply_button->setExtra( "onclick=\"location='pmlite.php?reply=1&amp;msg_id={$msg_id}'\"" );
...
$xoBreadcrumbs[] = array( 'title' => $link );
$xoBreadcrumbs[] = array( 'title' => $pm->getVar( 'subject' ) );
