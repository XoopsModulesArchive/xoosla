...
if ( empty( $xoopsConfigUser['allow_register'] ) ) {
	redirect_header( 'index.php', 1, _US_NOREGISTER );
	exit();
}
...
switch ( $op ) {
	case 'newuser':
		...
		if ( empty( $stop ) ) {
			...
			include $GLOBALS['xoops']->path( 'include/form/registerform.php' );
			$reg_form->display();
		}
	...
	case 'finish':
		...
		if ( empty( $stop ) ) {
			...
			$newuser->setVar( 'user_avatar', 'avatars/default.png', true );
	...
	case 'activate':
		...
		if ( $thisuser->getVar( 'actkey' ) != $actkey ) {
			redirect_header( 'index.php', 1, _US_ACTKEYNOT );
		} else {
			if ( $thisuser->getVar( 'level' ) > 0 ) {
				redirect_header( 'user.php', 1, _US_ACONTACT, false );
			} else {
				if ( false != $member_handler->activateUser( $thisuser ) ) {
					...
					if ( $xoopsConfigUser['activation_type'] == 2 ) {
						...
						redirect_header( 'user.php', 1, _US_ACTLOGIN, false );
					}
				} else {
					redirect_header( 'index.php', 1, _US_ACTFAILD );
				}
			}
		}
		break;
	...
	default:
		$xoopsOption['xoops_pagetitle'] = _US_USERREG;
		include $GLOBALS['xoops']->path( 'header.php' );
		$xoTheme->addMeta( 'meta', 'keywords', _US_USERREG . ', ' . _US_NICKNAME );
