...
class XoopsCommentRenderer {
	 ...
	 * @return
	 */
	public function instance( XoopsTpl &$tpl, $use_icons = true, $do_iconcheck = false )
	{
		static $instance;
		if ( !isset( $instance ) ) {
	 ...
	 */
	public function setComments( XoopsComment &$comments_arr )
	{
	...
	public function renderThreadView( $comment_id = 0, $admin_view = false, $show_nav = true )
	{
		...
		if ( false != $admin_view ) {
			...
			. '<div style="text-align:right; margin-top: 2px; margin-bottom: 0px; margin-right: 2px;">' . _CM_STATUS . ': ' . $this->_statusText[$tree[$comment_id]['obj']->getVar( 'com_status' )] . '<br />IP: <span style="font-weight: bold;">' . $tree[$comment_id]['obj']->getVar( 'com_ip' ) . '</span></div>';
		} else {
			// hide comments that are not active
