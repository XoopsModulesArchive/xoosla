...
';
	...
	foreach ( $lines as $line ) {
		$ret .= NWLINE . 'debug_window.document.writeln("' . str_replace( array( '"' , '</' ), array( '\"' , '<\/' ), $line ) . '");';
	}
	$ret .= '
    debug_window.focus();
...
if ( empty( $mode ) ) {
	...
	$ret .= '<div id="xo-logger-output"><div id="xo-logger-tabs">';
...
if ( empty( $mode ) || $mode == 'errors' ) {
	...
	$class = 'even';
	$ret .= '<table id="xo-logger-errors"><tr><th>' . _LOGGER_ERRORS . '</th></tr>';
	foreach ( $this->errors as $error ) {
		$ret .= NWLINE.'<tr><td class="'.$class.'">';
...
if ( empty( $mode ) || $mode == 'deprecated' ) {
	$class = 'even';
	$ret .= '<table id="xo-logger-deprecated"><tr><th>' . _LOGGER_DEPRECATED . '</th></tr>';
	foreach ( $this->deprecated as $message ) {
		$ret .= '<tr><td class="'.$class.'">';
		$ret .= $message;
		$ret .= '<br />\n</td></tr>';
		$class = ( $class == 'odd' ) ? 'even' : 'odd';
	}
...
if ( empty( $mode ) || $mode == 'queries' ) {
	$class = 'even';
	$ret .= '<table id="xo-logger-queries"><tr><th>' . _LOGGER_QUERIES . '</th></tr>';
...
if ( empty( $mode ) || $mode == 'blocks' ) {
	$class = 'even';
	$ret .= '<table id="xo-logger-blocks"><tr><th colspan="2">' . _LOGGER_BLOCKS . '</th></tr>';
...
if ( empty( $mode ) || $mode == 'extra' ) {
	$class = 'even';
	$ret .= '<table id="xo-logger-extra"><tr><th colspan="2">' . _LOGGER_EXTRA . '</th></tr>';
...
if ( empty( $mode ) || $mode == 'timers' ) {
	$class = 'even';
	$ret .= '<table id="xo-logger-timers"><tr><th colspan="2">' . _LOGGER_TIMERS . '</th></tr>';
