                        ...
                        if ( !in_array( $prefixed_query[4], $reservedTables ) ) {
                            ...
                            if ( !$db->query( $prefixed_query[0] ) ) {
                                ...
                                if ( !in_array( $prefixed_query[4], $created_tables ) ) {
                                    ...
                                    $msgs[] = "&nbsp;&nbsp;" . sprintf( _MD_AM_SQL_NOT_FOUND, "<strong>" . $db->prefix( $prefixed_query[4] ) . "</strong>" );
                                }
                            }
                        } else {
...
function &xoops_module_gettemplate( $dirname, $template, $block = false )
{
	...
	$path = XOOPS_THEME_PATH . '/' . $xoopsConfig['theme_set'] . '/modules/' . $dirname . '/';
    if ( $block == true ) {
        $path .= 'blocks/';
    }
    $path .= $template;
	...
	*/
    if ( !file_exists( $path ) ) {
        unset( $path );
        $path = XOOPS_THEME_PATH . '/default/modules/' . $dirname . '/';
        if ( $block == true ) {
            $path .= 'blocks/';
        }
        $path .= $template;
            ...
            if ( $modtables != false && is_array( $modtables ) ) {
                ...
                foreach ( $modtables as $table ) {
                    ...
                    if ( !in_array( $table, $reservedTables ) ) {
                        ...
                        if ( !$db->query( $sql ) ) {
                            $msgs[] = '&nbsp;&nbsp;<span style="color:#ff0000;">' . sprintf( _MD_AM_TABLE_DROPPED_ERROR, "<strong>" . $db->prefix( $table ) . "<strong>" );
                        } else {
