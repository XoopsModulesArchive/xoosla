...
class XoopsObject
{
    ...
    */
    function getVar($key, $format = 's')
    {
        ...
        case XOBJ_DTYPE_LTIME:
            switch ( strtolower( $format ) ) {
                case 's':
                    $var = '';
					$time = strtotime( gmdate( 'd/m/Y', (int)$ret ) );
					if ($time > -3600 ) {
						$var = formatTimeStamp( $ret );
					} else{
						$var = '------';
					}
                    return $var;
    ...
    function cleanVars()
    {
           //trigger_error(__CLASS__.'::'.__FUNCTION__ . ' is deprecated.', E_USER_NOTICE);
        $ts =& MyTextSanitizer::getInstance();
            ...
            if (!$v['changed']) {
                ...
                case XOBJ_DTYPE_EMAIL:
                    ...
                    if ($cleanv != '' && !preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+([\.][a-z0-9-]+)+$/i",$cleanv)) {
                        /**
                        * Language Fix: Catzwolf
                        **/
						$this->setErrors( sprintf( _XOBJ_ERR_EMAIL, '%s' ) );
                        continue;
                    }
                    if (!$v['not_gpc']) {
                        $cleanv = $ts->stripSlashesGPC($cleanv);
