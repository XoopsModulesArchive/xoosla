...
function xoops_inlude( $fullpath = '', $root = null, $type = 1 )
{
    if ( !class_exists( 'XoopsLoad' ) ) {
        require_once XOOPS_ROOT_PATH . '/class/xoopsload.php';
    }
    return XoopsLoad::loadInclude( $fullpath, $root, $type );
}

function xoops_language( $xo_language_file, $xo_isModule = false )
{
    if ( !class_exists( 'XoopsLoad' ) ) {
        xoops_inlude( '/class/xoopsload' );
    }
    return XoopsLoad::loadLanguage( $xo_language_file, $xo_isModule );
}
...
function xoops_header( $closehead = true )
{
    ...
    <script type="text/javascript" src="' . XOOPS_URL . '/include/javascript/xoops.js"></script>
    ';
...
function redirect_header( $url, $time = 3, $message = '', $addredirect = true, $allowExternalLink = false )
{
    ...
    $xoopsTpl->assign( array( 'xoops_theme' => $theme,
...
function &xoops_gethandler( $name, $optional = false )
{
    ...
    if ( !isset( $handlers[$name] ) && !$optional ) {
        trigger_error( 'Class <strong>' . $class . '</strong> does not exist<br />Handler Name: ' . $name, E_USER_ERROR );
    }
    if ( isset( $handlers[$name] ) ) {
 ...
 * @return
 */
function xoops_cleanRequestVars( &$array, $key, $def = null, $type = 0, $length = 255, $gpc = false, $strip = false, $allowed_tags = array() )
{
    ...
    switch ( intval( $type ) ) {
        case XOBJ_DTYPE_TXTBOX:
            ...
            return stripslashes( xoops_trim( $value ) );
            break;
        case XOBJ_DTYPE_TXTAREA:
            return stripslashes( xoops_trim( $value ) );
            break;
        case XOBJ_DTYPE_INT:
            $value = ( ( int )$value <= 0 ) ? 0 : intval( $value );
            return $value;
            break;
        case XOBJ_DTYPE_URL:
            if ( $value != '' && !preg_match( "/^http[s]*:\/\//i", xoops_trim( $value ) ) ) {
                $value = 'http://' . $value;
            }
            return stripslashes( xoops_trim( $value ) );
            break;
...
xoops_inlude( 'include.functions_legacy' );
