...
class CriteriaCompo extends CriteriaElement {
     ...
     * @var array
     */
    var $criteria_start = array();
    /**
     * end
     *
     * @var array
     */
    var $criteria_end = array();
    /**
     * Constructor
     *
     * @param object $ele
     ...
     */
    function CriteriaCompo( $ele = null, $condition = 'AND', $criteria_start = false, $criteria_end = true ) {
        if ( isset( $ele ) && is_object( $ele ) ) {
            $this->add( $ele, $condition, $criteria_start, $criteria_end );
        }
    }
     ...
     */
    function &add( &$criteriaElement, $condition = 'AND', $criteria_start = false, $criteria_end = false ) {
        $this->criteriaElements[] = &$criteriaElement;
        $this->conditions[] = $condition;
        $this->criteria_start[] = $criteria_start;
        $this->criteria_end[] = $criteria_end;
        return $this;
    }
	/**
	* Start Catzwolf Change
	**/
    /**
    ...
    function render() {
        ...
        if ( $count > 0 ) {
            ...
            for ( $i = 1; $i < $count; $i++ ) {
                ...
                if ( isset( $this->criteria_start[$i] ) ) {
                    $ret .= $this->criteria_start[$i];
                }
                $ret .= $this->criteriaElements[$i]->render();
                if ( isset( $this->criteria_end[$i] ) ) {
                    $ret .= $this->criteria_end[$i];
                }
            }
            $ret .= ' ';
        }
