 ...
 * @version $Id: xoopstextsanitizer.php 1536 2008-05-02 04:23:55Z phppp $
 */
...
class MyTextSanitizerExtension {
    ...
    /*
    function __construct(& $myts)
    {
        $this->ts =& $myts;
    }

    function MyTextSanitizerExtension($myts)
    {
        $this->__construct($myts);
    }
...
class MyTextSanitizer {
    ...
    {
        $this->path_basic = 'plugins.textsanitizer';
        $this->path_plugin = 'frameworks.textsanitizer';
    ...
    */
    ...
    {
        $filename = empty( $filename ) ? 'config' : 'config_' . ( string )$filename;
        $config = xoops_inlude( $this->path_basic . XD_S . $filename, null, 1, true );
        $config_plugin = xoops_inlude( $this->path_plugin . XD_S . $filename, null, 1, true );
        if ( is_array( $config_plugin ) ) {
            foreach ( $config_plugin as $key => $val ) {
                /* We could be over writing a key already. No check to see if the key exists? */
                $config[$key] = $val;
            }
        }
        return $config;
    ...
    function makeClickable( &$text )
    {
        ...
        $patterns = array( "/(^|[^]_a-z0-9-=\"'\/])([a-z]+?):\/\/([{$valid_chars}]+)/ei",
            "/(^|[^]_a-z0-9-=\"'\/])www\.([a-z0-9\-]+)\.([{$valid_chars}]+)/ei",
            "/(^|[^]_a-z0-9-=\"'\/])ftp\.([a-z0-9\-]+)\.([{$valid_chars}]+)/ei",
            "/(^|[^]_a-z0-9-=\"'\/:\.])([a-z0-9\-_\.]+?)@([{$valid_chars}]+)/ei" );
        $replacements = array( "'\\1<a href=\"\\2://\\3\" title=\"\\2://\\3\" rel=\"external\">\\2://'.MyTextSanitizer::truncate( '\\3' ).'</a>'",
    ...
    */
    ...
    {
        $ret = $this->loadExtension( 'censor', $text );
        if ( $ret === false ) {
            return $text;
        }
        return $ret;
    ...
    function codeConv( $text, $xcode = 1 )
    {
        ...
        $patterns = "/\[code([^\]]*?)\](.*)\[\/code\]/esU";
        $replacements = "'<div class=\"xoopsCode\"><code>'.\$this->loadExtension('syntaxhighlight', \$this->htmlSpecialChars(str_replace('\\\"', '\"', base64_decode('$2'))), '$1').'</code></div>'";
        $text = preg_replace( $patterns, $replacements, $text );
        return $text;
    }
    /**
    ...
    {
        if ( empty( $this->config['extensions'] ) ) return true;
        foreach ( $this->config['extensions'] as $extension => $enabled ) {
            if ( $enabled ) $this->loadExtension( $extension );
        }
    }

    function loadExtension( $name )
    {
        if ( !xoops_inlude( $this->path_basic . XD_S . $name ) ) {
            if ( !xoops_inlude( $this->path_plugin . XD_S . $name ) ) {
                return false;
            }
        }
        $class = 'Myts' . ucfirst( $name );
        if ( ! class_exists( $class ) ) {
            trigger_error( 'Extension ' . $name . ' not exist in class ' . __CLASS__, E_USER_WARNING );
            return false;
        }
    ...
    {
        $ret = $this->loadExtension( 'textfilter', $text, $force );
        if ( $ret === false ) {
            return $text;
        }
        return $ret;
