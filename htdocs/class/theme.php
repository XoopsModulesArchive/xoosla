 ...
 *
 * @copyright       he XOOPS Project http://sourceforge.net/projects/xoops/
...
class xos_opal_Theme {
   ...
   function xoInit( $options = array() ) {
		...
		$this->template->assign( array(
            ...
            'xoops_footer' => !empty( $xoopsConfig['footer'] ) ? $xoopsConfig['footer'] : '&nbsp',
            'xoops_moduledir' => isset($xoopsModule) && is_object($xoopsModule) ? $xoopsModule->getVar('dirname') : '',
		...
		/**
		* Changed the method in which meta tags are taken from the database. $config_handler->getConfigsByCat now accepts an array as its arguement
		* so we can take more than one config category at a time. Hence the sql drop by one and there shouldn't be a
		* performace drop doing it this way either. :) Catzwolf
		**/
		foreach ( $xoopsConfig as $key => $value ) {
			if ( substr( $key, 0, 5 ) == 'meta_' ) {
				$this->addMeta( 'meta', substr( $key, 5 ), $value );
            }
        }
