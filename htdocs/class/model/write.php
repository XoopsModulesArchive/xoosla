        ...
        foreach ($vars as $k => $v) {
            ...
            switch ($v["data_type"]) {
                case XOBJ_DTYPE_TXTBOX:
                    ...
                    $cleanv = $this->handler->db->quoteString($cleanv);
                    break;
                ...
                case XOBJ_DTYPE_LTIME:
                    /**
                    * Note: Changed this to allow for use with the calender. Catzwolf
                    **/
					if ( is_array( $cleanv ) ) {
                        $date = ( !empty( $cleanv['date'] ) ) ? $cleanv['date']: '';
                        $time = ( !empty( $cleanv['time'] ) ) ? $cleanv['time']: '';
						$cleanv = strtotime( $date.' '.$time );
                    } else if ( is_int( $cleanv ) ) {
                        $cleanv = intval( $cleanv );
                    } else {
...
#                    $cleanv = !is_string($cleanv) ? intval($cleanv) : strtotime($cleanv);
    ...
    {
        if (!$object->isDirty()) {
            trigger_error("Insert failed - the object '" . get_class($object). "' is not dirty", E_USER_WARNING);
        ...
        if ($object->isNew()) {
            ...
            if (!empty($object->cleanVars)) {
                ...
                trigger_error("Insert failed - no cleanVars of object '" . get_class($object). "'", E_USER_WARNING);
                return $object->getVar($this->handler->keyName);
            }
            ...
            $sql = "UPDATE " . $this->handler->table . " SET ".implode(",",$keys)." WHERE ".$this->handler->keyName." = " . $this->handler->db->quoteString($object->getVar($this->handler->keyName));
            if (!$result = $this->handler->db->{$queryFunc}($sql)) {
                return false;
            }
        }
    ...
    function delete(&$object, $force = false)
    {
        if (is_array($this->handler->keyName)) {
            ...
            for ($i = 0; $i < count($this->handler->keyName); $i++) {
                $clause[] = $this->handler->keyName[$i]." = ".$this->handler->db->quoteString($object->getVar($this->handler->keyName[$i]));
            }
            $whereclause = implode(" AND ", $clause);
        } else {
            $whereclause = $this->handler->keyName." = ".$this->handler->db->quoteString($object->getVar($this->handler->keyName));
        }
    ...
    function updateAll($fieldname, $fieldvalue, $criteria = null, $force = false)
    {
        ...
        if ( is_numeric( $fieldvalue ) ) {
            ...
            $set_clause .= $this->handler->db->quoteString( implode( ',', $fieldvalue ) );
        } else {
            $set_clause .= $this->handler->db->quoteString( $fieldvalue );
        }
