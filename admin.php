...
if ( $xoopsUser ) {
	if ( !$xoopsUser->isAdmin( - 1 ) ) {
		redirect_header( 'index.php', 1, _AD_NORIGHT );
		exit();
	}
} else {
	redirect_header( 'index.php', 1, _AD_NORIGHT );
...
if ( !empty( $_GET['xoopsorgnews'] ) ) {
	// Multiple feeds
	$rssurl = array();
	$rssurl[] = 'http://www.xoosla.com/backend.php';

	$urls = xoops_loadLanguage( 'backend' );
	if ( !empty( $urls ) ) {
		...
		foreach ( $rssurl as $url ) {
			if ( $snoopy->fetch( $url ) ) {
				...
				if ( false != $rss2parser->parse() ) {
					...
					$count = count( $_items );
					for ( $i = 0; $i < $count; $i ++ ) {
						$_items[$i]['title'] = XoopsLocal::ConvertEncoding( $_items[$i]['title'], _CHARSET, 'UTF-8' );
						$_items[$i]['description'] = XoopsLocal::ConvertEncoding( $_items[$i]['description'], _CHARSET, 'UTF-8' );
