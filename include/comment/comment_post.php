 ...
 * @version $Id: comment_post.php 26 2012-02-17 09:16:15Z catzwolf $
 */
...
switch ( $op ) {
	case "delete":
		include_once $GLOBALS['xoops']->path( 'include/comment/comment_delete.php' );
		break;
	...
	case "preview":
		...
		if ( $xoopsModule->getVar( 'dirname' ) != 'system' ) {
			...
			echo '<table cellpadding="2" cellspacing="1" width="98%" class="outer">
                  <tr><td class="head">' . $com_title . '</td></tr>
                  <tr><td><br />' . $p_comment . '<br /></td></tr>
                  </table>';
			include_once $GLOBALS['xoops']->path( 'include/comment/comment_form.php' );
		...
		if ( false != $comment_handler->insert( $comment ) ) {
			...
			if ( false != $call_updatefunc && isset( $comment_config['callback']['update'] ) && trim( $comment_config['callback']['update'] ) != '' ) {
				...
				if ( !$skip ) {
					...
					call_user_func_array( $func, array( $com_itemid ,
							$comment_count ,
