				...
				if ( $smiles = $myts->getSmileys() ) {
					...
					foreach ( $smiles as $key => $smile ) {
						echo '<tr class="'.$rcolor.'">
							...
							<td>' . $smile['emotion'] . '</td>
							<td>';
						echo "<img onmouseover='style.cursor=\"hand\"' onclick='doSmilie(\" " . $smile['code'] . " \");' src=\"" . XOOPS_UPLOAD_URL . "/" . $smile['smile_url'] . "\" alt=\"\" />";
						echo '</td>
						</tr>';
	...
	<tr>
		...
		case 'friend':
			if ( !$GLOBALS['xoopsSecurity']->check() || ! isset( $_POST['op'] ) || $_POST['op'] == 'sendform' ) {
				...
				echo '<form action="' . XOOPS_URL . '/misc.php" method="post" onsubmit="return checkForm();"><table  width="100%" class="outer" cellspacing="1"><tr><th colspan="2">' . _MSC_RECOMMENDSITE . '</th></tr>';
                ...
                <tr><td class="head">&nbsp;</td><td class="even"><input type="submit" value="' . _SAVE . '" />&nbsp;<input value="' . _CLOSE . '" type="button" onclick="javascript:window.close();" />' . $GLOBALS["xoopsSecurity"]->getTokenHTML() . '</td></tr>
                </table></form>';
				$closebutton = 0;
				...
				if ( !isset( $_POST['yname'] ) || trim( $_POST['yname'] ) == "" || $ymail == '' || !isset( $_POST['fname'] ) || trim( $_POST['fname'] ) == "" || !isset( $_POST['fmail'] ) || trim( $_POST['fmail'] ) == '' ) {
					redirect_header( XOOPS_URL . '/misc.php?action=showpopups&amp;type=friend&amp;op=sendform', 1, _MSC_NEEDINFO );
					exit();
				}
				...
				if ( !checkEmail( $fmail ) || !checkEmail( $ymail ) || preg_match( "/[\\0-\\31]/", $yname ) ) {
					...
					redirect_header( XOOPS_URL . '/misc.php?action=showpopups&amp;type=friend&amp;op=sendform', 1, $errormessage );
					exit();
				}
		...
		case 'online':
			...
			for ( $i = 0; $i < $count; $i++ ) {
				...
				if ( is_object( $onlineUsers[$i]['user'] ) ) {
					...
					echo '<td>' . $avatar . '</td>';
					echo "<td><a href=\"javascript:window.opener.location='" . XOOPS_URL . "/userinfo.php?uid=" . $onlineUsers[$i]['user']->getVar( 'uid' ) . "';window.close();\">" . $onlineUsers[$i]['user']->getVar( 'uname' ) . "</a>";
				} else {
