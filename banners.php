 ...
 * @version $Id$
 */
require dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'mainfile.php';
...
function clientlogin()
{
	...
	$GLOBALS['xoTheme']->addStylesheet( null, null, '
        ...
        #login_window  h2 {
            margin:                             .5em;
            padding:                            130px 0 0;
            background:                         url( images/form/password.png) no-repeat center top;
            text-align:                         center;
        }
...
function bannerstats()
{
	...
	if ( $_SESSION['banner_login'] == '' || $_SESSION['banner_pass'] == '' ) {
		redirect_header( 'banners.php', 1, _BANNERS_NO_LOGIN_DATA );
		exit();
	}
	...
	if ( $_SESSION['banner_pass'] == $passwd ) {
		...
		$GLOBALS['xoTheme']->addStylesheet( null, null, '#bannerstats{}; #bannerstats td {text-align: center;};' );
		...
		echo '</table>
              <br /><br />
              <h4 class="content_title">' . _BANNERS_FOW_IN . htmlspecialchars( $xoopsConfig['sitename'] ) . '</h4>';
...
function EmailStats( $cid, $bid )
{
	...
	if ( $_SESSION['banner_login'] != '' && $_SESSION['banner_pass'] != '' ) {
		...
		if ( $result2 = $xoopsDB->query( sprintf( 'SELECT name, email, passwd FROM %s WHERE cid=%u AND login=%s', $xoopsDB->prefix( 'bannerclient' ), $cid, $xoopsDB->quoteString( $_SESSION['banner_login'] ) ) ) ) {
			...
			if ( $_SESSION['banner_pass'] == $passwd ) {
				if ( $email == '' ) {
					redirect_header( 'banners.php', 1, sprintf( _BANNERS_MAIL_ERROR, $name ) );
					exit();
				} else {
					if ( $result = $xoopsDB->query( 'SELECT bid, imptotal, impmade, clicks, imageurl, clickurl, date FROM ' . $xoopsDB->prefix( 'banner' ) . ' WHERE bid=' . $bid . ' AND cid=' . $cid ) ) {
						...
						$xoopsMailer->send();
						redirect_header( 'banners.php?op=Ok', 1, _BANNERS_MAIL_OK );
						exit();
					}
				}
			}
		...
		if ( $result = $xoopsDB->query( $sql ) ) {
			...
			if ( $_SESSION['banner_pass'] == $passwd ) {
				...
				if ( $xoopsDB->query( $sql ) ) {
					redirect_header( 'banners.php?op=Ok', 1, _BANNERS_DBUPDATED );
					exit();
				}
			}
		}
...
function clickbanner( $bid )
{
	...
	redirect_header( XOOPS_URL, 1, _BANNERS_NO_ID );
	exit();
}
	...
	case 'Ok':
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
			if ( !$GLOBALS['xoopsSecurity']->check( true, false, 'BANNER_LOGIN' ) ) {
				redirect_header( 'banners.php', 1, implode( '<br />', $GLOBALS['xoopsSecurity']->getErrors() ) );
				exit();
			}
